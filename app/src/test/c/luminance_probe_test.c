...
TEST("Bipartite Luminance Probe", bipartite_luminance_probe) {
    ...
    for (unsigned i = 0; i < 32; ++i) {
        if (left[i] > 0) {
            vec2_t pt = luminance_probe_left(&image, i);
            assert_that(float_equal(pt.x, left[i], 0.01f) && float_equal(pt.y, (float) i, 0.01f), "Probe left (y = %d) expected (%.0f, %d), but got (%.0f, %.0f)", i, left[i], i, pt.x, pt.y);
        }
        if (right[i] > 0) {
            vec2_t pt = luminance_probe_right(&image, i);
            assert_that(float_equal(pt.x, right[i], 0.01f) && float_equal(pt.y, (float) i, 0.01f), "Probe right (y = %d) expected (%.0f, %d), but got (%.0f, %.0f)", i, right[i], i, pt.x, pt.y);
        }
    }
    for (unsigned i = 0; i < 48; ++i) {
        if (top[i] > 0) {
            vec2_t pt = luminance_probe_top(&image, i);
            assert_that(float_equal(pt.x, (float) i, 0.01f) && float_equal(pt.y, top[i], 0.01f), "Probe top (x = %d) expected (%d, %.0f), but got (%.0f, %.0f)", i, i, top[i], pt.x, pt.y);
        }
        if (bottom[i] > 0) {
            vec2_t pt = luminance_probe_bottom(&image, i);
            assert_that(float_equal(pt.x, (float) i, 0.01f) && float_equal(pt.y, bottom[i], 0.01f), "Probe bottom (x = %d) expected (%d, %.0f), but got (%.0f, %.0f)", i, i, bottom[i], pt.x, pt.y);
        }
    }
    free_bitmap(&image);
    pass();
